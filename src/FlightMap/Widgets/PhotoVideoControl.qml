...
Rectangle {
    ...
    // This used to carry a separate "Record Stream" button, because setting CAM1_TYPE=MAVLink made ArduPilot
    // announce itself as a photo-only camera: QGC then dropped SimulatedCameraControl for VehicleCameraControl,
    // capturesVideo() went false, and the ordinary record button - the only path that writes the incoming stream
    // to disk - disappeared. The fix now lives in the firmware instead: AP_Camera_MAVLink no longer answers
    // CAMERA_INFORMATION, so QGC never sees a MAVLink camera and the stock control works unmodified. Keeping our
    // own button on top of that would just be a second, divergent way to do the same thing.
